...
export default function VocabTab({ 
  ...
  const handleAddWord = async () => {
    if (addMode === "single") {
      ...
      try {
        ...
        if (!handled) alert(t.vocabErrorAnalyze);
      } finally {
        setIsAdding(false);
      }
      ...
      try {
        ...
        const handled = await onError(error);
        if (!handled) alert(t.vocabErrorExtract);
      } finally {
        setIsAdding(false);
      }
    }
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const handled = await onError(error);
      if (!handled) alert(t.vocabErrorScan);
      setIsAdding(false);
    }
  };
  ...
  const deleteWordByWord = (item: Vocabulary) => {
    if (confirm(t.vocabConfirmDelete.replace("{word}", item.chinese))) {
      setVocabList(prev => prev.filter(v => v.id !== item.id));
    }
  };

  const deleteAll = () => {
    if (confirm(t.vocabConfirmDeleteAll)) {
      setVocabList([]);
    }
  };
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div className="flex bg-neutral-200 p-1 rounded-xl">
              <button 
                ...
                title={t.vocabFilterStatus}
              >
                <Filter className="w-4 h-4" />
                <span className="text-[10px] font-bold uppercase">
                  {filterStatus === "all" ? t.vocabFilterAll : filterStatus === "mastered" ? t.vocabFilterMastered : t.vocabFilterUnmastered}
                </span>
              </button>
              <button 
                ...
                title={t.vocabSortOrder}
              >
                {sortOrder === "newest" ? <Clock className="w-4 h-4" /> : <SortAsc className="w-4 h-4" />}
                <span className="text-[10px] font-bold uppercase">
                  {sortOrder === "newest" ? t.vocabSortNewest : t.vocabSortAlpha}
                </span>
              </button>
            </div>
          </div>
          ...
          <button 
            ...
            title={t.vocabUploadToSheet}
          >
            <Upload className="w-4 h-4" />
            {t.vocabUploadToSheet}
          </button>
        </div>
        ...
        <div className="space-y-2">
          {WORD_TYPES.length > 0 && (
            <div className="flex flex-wrap gap-2">
              <span className="text-[10px] font-bold text-neutral-400 uppercase py-1">{t.vocabWordType}:</span>
              {WORD_TYPES.map(type => (
                <button
                  key={type}
          ...
          {TOPICS.length > 0 && (
            <div className="flex flex-wrap gap-2">
              <span className="text-[10px] font-bold text-neutral-400 uppercase py-1">{t.vocabTopic}:</span>
              {TOPICS.map(topic => (
                <button
                  key={topic}
      ...
      <AnimatePresence mode="wait">
        {viewMode === "table" ? (
          ...
          <FlashcardView 
            ...
            emptyMessage={t.vocabNoWordsToLearn}
          />
        )}
      </AnimatePresence>
        ...
        <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-6 backdrop-blur-sm">
          ...
          >
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-bold">{t.vocabEdit}</h3>
              <button onClick={() => setShowEditModal(false)}><X className="w-6 h-6" /></button>
            </div>
            <div className="space-y-4">
              <div>
                <label className="text-xs font-bold text-neutral-400 uppercase">{t.vocabChineseChar}</label>
                <input 
                  type="text" 
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-bold text-neutral-400 uppercase">{t.vocabPinyin}</label>
                  <input 
                    type="text" 
                ...
                <div>
                  <label className="text-xs font-bold text-neutral-400 uppercase">{t.vocabAmBoi}</label>
                  <input 
                    type="text" 
              ...
              <div>
                <label className="text-xs font-bold text-neutral-400 uppercase">{t.vocabMeaning}</label>
                <input 
                  type="text" 
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-bold text-neutral-400 uppercase">{t.vocabHanViet}</label>
                  <input 
                    type="text" 
                ...
                <div>
                  <label className="text-xs font-bold text-neutral-400 uppercase">{t.vocabWordType}</label>
                  <input 
                    type="text" 
              ...
              <div>
                <label className="text-xs font-bold text-neutral-400 uppercase">{t.vocabTopic}</label>
                <input 
                  type="text" 
        ...
        <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-6 backdrop-blur-sm">
          ...
          >
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-lg font-bold">{t.vocabAddNew}</h3>
            ...
            <div className="flex bg-neutral-100 p-1 rounded-2xl mb-6">
              ...
              >
                <Plus className="w-4 h-4" /> {t.vocabSingleWord}
              </button>
              <button 
              ...
              >
                <TypeIcon className="w-4 h-4" /> {t.vocabParagraph}
              </button>
              <button 
              ...
              >
                <ImageIcon className="w-4 h-4" /> {t.vocabImage}
              </button>
            </div>
            ...
            {addMode === "single" && (
              <input 
                ...
                placeholder={t.vocabEnterChinese}
            ...
            {addMode === "text" && (
              <textarea 
                ...
                placeholder={t.vocabPasteParagraph}
            ...
            {addMode === "image" && (
              ...
              >
                ...
                <span className="text-sm text-neutral-500 font-medium">{t.vocabClickToUploadImage}</span>
                <input 
                  type="file" 
                  ref={fileInputRef}
...
function TableView({ list, onToggleMastered, onDelete, onEdit, onSelect, emptyMessage }: { list: Vocabulary[], onToggleMastered: (word: string) => void, onDelete: (item: Vocabulary) => void, onEdit: (item: Vocabulary) => void, onSelect: (item: Vocabulary) => void, emptyMessage: string, key?: string }) {
  return (
    <motion.div 
    ...
    >
      {list.map((item, i) => (
        ...
        >
          ...
          <div className="flex items-center gap-1">
            ...
            <button onClick={() => onEdit(item)} className="p-2 text-neutral-300 hover:text-emerald-500 rounded-lg hover:bg-neutral-50">
              <Edit2 className="w-4 h-4" />
            </button>
            <button onClick={() => onDelete(item)} className="p-2 text-neutral-300 hover:text-red-500 rounded-lg hover:bg-red-50">
      ...
      {list.length === 0 && (
        <div className="text-center py-20 text-neutral-400">{emptyMessage}</div>
      )}
    </motion.div>
  );
}

function FlashcardView({ list, onToggleMastered, onEdit, onDelete, initialIndex = 0, emptyMessage }: { list: Vocabulary[], onToggleMastered: (word: string) => void, onEdit: (item: Vocabulary) => void, onDelete: (item: Vocabulary) => void, initialIndex?: number, emptyMessage: string, key?: string }) {
  ...
  if (!currentItem) return <div className="text-center py-20 text-neutral-400">{emptyMessage}</div>;
