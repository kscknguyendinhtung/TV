...
export default function GrammarTab({ points, setPoints, onUpload, isSyncing }: Props) {
  ...
  const deletePoint = (index: number) => {
    if (confirm(t.grammarConfirmDelete)) {
  ...
  const handleOrderingClick = (word: string) => {
    ...
    if (userOrdering.includes(word)) {
      ...
      if (newOrdering.length === quizQuestions[currentQuizIndex].options.length) {
        ...
        if (normalize(finalSentence) === normalize(correctAns) || newOrdering.join("") === correctAns.replace(/\s+/g, "")) {
          setScore(prev => prev + 1);
        }
        setShowExplanation(true);
      }
    ...
    >
      <div className="flex items-center justify-between mb-6">
        <div className="text-left space-y-1">
          <h2 className="text-2xl font-bold text-neutral-800">{t.grammarTitle}</h2>
          <p className="text-neutral-500 text-sm">{t.grammarSubtitle}</p>
        </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <div className="text-xs font-bold text-neutral-400 uppercase tracking-widest">{t.grammarExample}</div>
                  <button 
            ...
            >
              ...
              <div className="mb-8">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-bold text-emerald-600 uppercase tracking-widest">{t.grammarQuestion.replace("{current}", String(currentQuizIndex + 1)).replace("{total}", String(quizQuestions.length))}</span>
                  <span className="text-xs font-bold text-neutral-400">{t.grammarCorrect.replace("{count}", String(score))}</span>
                </div>
              ...
              <div className="space-y-6">
                <div className="text-center space-y-2">
                  ...
                  <div className="text-3xl font-bold text-neutral-800">
                    {quizQuestions[currentQuizIndex].type === "ordering" ? t.grammarOrderingInstruction : quizQuestions[currentQuizIndex].question}
                  </div>
                </div>
                ...
                {quizQuestions[currentQuizIndex].type === "ordering" && (
                  <div className="min-h-[60px] p-4 bg-neutral-50 rounded-2xl border-2 border-dashed border-neutral-200 flex flex-wrap gap-2 items-center justify-center">
                    ...
                    {userOrdering.length === 0 && <span className="text-neutral-300 text-sm">{t.grammarNoWordsSelected}</span>}
                  </div>
                )}
                ...
                {showExplanation && (
                  ...
                  >
                    <div className="text-xs font-bold text-blue-600 uppercase mb-1">{t.grammarExplanation}</div>
                ...
                {selectedAnswer && (
                  ...
                  >
                    {currentQuizIndex === quizQuestions.length - 1 ? t.grammarFinish : t.grammarNext}
                  </button>
                )}
              </div>
            </motion.div>
